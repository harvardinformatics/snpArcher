...
set.colors <- c('#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c','#fdbf6f','#ff7f00','#cab2d6','#6a3d9a')
```
...
plot(nj.dist, tip.color = mycol, type = "fan")

#p <- ggtree(nj.dist, layout = "fan")
#p %<+% dist.id + geom_tiplab(pch = 16, aes(col = cluster)) +
...
#sample.order <- cat_snmf %>% 
#  group_by(sampleID) %>% 
#  filter(prob == max(prob)) %>% 
#  arrange(popGroup) %>% 
#  ungroup() %>% 
#  mutate(sample_order = 1:n()) %>% 
#  select(sampleID, sample_order)

##############################
cat_snmf.wide <- cat_snmf %>% 
  select(sampleID, k, popGroup, prob) %>% 
  pivot_wider(names_from = "popGroup", values_from = "prob") %>% 
  group_by(k) %>% 
  arrange(k, pop1, pop2, pop3) %>% 
  mutate(sample_order = 1:n()) %>% 
  ungroup()

sample.order <- cat_snmf.wide %>% select(sampleID, sample_order, k)

cat_snmf <- left_join(cat_snmf, sample.order, by = c("sampleID", "k"))
##############################
...
p.k2 <- cat_snmf %>% 
  ...
  scale_fill_manual(name = "grp",values = set.colors, guide = F) +xlab(NULL)  + theme(legend.position = "none")
...
if(file.exists(paste0(prefix, ".coords.txt"))){
  ...
  df.coords$long.jit <- jitter(df.coords$long, 8)
  df.coords$lat.jit <- jitter(df.coords$lat, 8)
  
 plot_geo(df.coords, lat = ~lat.jit, lon = ~long.jit) %>% 
      layout(legend = list(orientation = 'h'), geo = g) %>% 
   add_markers(
      text = ~paste(IID, cluster, PC1, PC2, sep = "<br />"),
      color = ~cluster, symbol = I("circle"), size = I(30), 
