...
```{r, fig.width = 5, fig.height = 5}
# bamstat plot --------------------------------------------------------------
#bamstat is read in the PCA chunk
...
if(file.exists(paste0(prefix, ".coords.txt"))){
  if(!is.null(input$GMKey)){
    df.coords <- read.table(paste0(prefix, ".coords.txt"), na.strings = "")
    names(df.coords) <- c("sample.ID","long","lat")
    
    df.coords <- left_join(df.pca, df.coords, by = c("IID" = "sample.ID"))

    df.coords <- df.coords %>% filter(!na(long))
    num.missing <- df.coords %>% filter(is.na(long)) 
...
`r num.missing` dropped that are missing coordinates 
