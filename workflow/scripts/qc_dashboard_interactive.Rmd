...
#prefix <- "/Volumes/GoogleDrive/My\ Drive/SantaCruz/Projects/Variant_pipeline/data/anenome_qc/anemone_short_jaAntSola1.0.p_ctg"
...
```{r}
nums.nps <- max(read.table(paste0(prefix, ".idepth"), header = T)$N_SITES)
...
if(length(l.inf)==0){
  ...
  df.rel.long$Sample2 <- factor(x = df.rel.long$Sample2,
                                 levels = dist.id$V2[sample.order], 
                                 ordered = TRUE)
...
if(file.exists(paste0(prefix, ".coords.txt"))){
 ...
 plot_geo(df.coords, lat = ~lat, lon = ~long) %>% 
   add_markers(
      text = ~paste(IID, cluster, PC1, PC2, sep = "<br />"),
      color = ~cluster, symbol = I("circle"), size = I(30), hoverinfo = "text", colors = set.colors[1:input$clusters]
    ) %>% 
