...
render_rohplots <- function(project){
  ...
  load_file <- function(filename) {
    df <- read_tsv(filename, col_names=TRUE)
  ...
  # get top 2 samples -------------------------------------------------------
  topF <- read_tsv(topF, col_names = FALSE, show_col_types = F)
  topF$X1 <- gsub("#", "_", topF$X1)
  ...
  BCFfinal <- read_tsv(inputFile, col_names = c("group", "id", "chrom", "from", "to", "lengthBps", "nSNP", "Quality"),
                                  col_types = cols(group = "c", id = "c", chrom = "c", from ="n", to = "n", lengthBps = "n", nSNP = "n", Quality = "n"))   
  head(BCFfinal)
  BCFfinal$id <- gsub("#", "_", BCFfinal$id)

  BCFfinal$chrom <- gsub("Chr", "", BCFfinal$chrom)
  df_runs = BCFfinal[, c("group", "id", "chrom", "nSNP", 
                         "from", "to", "lengthBps")]
  df_runs <- as.data.frame(df_runs)
  df_runs <- df_runs %>%
